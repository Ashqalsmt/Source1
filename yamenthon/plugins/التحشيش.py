...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/def_Zoka) 🗑♻️𓆪\n**𓍹━─━─━─𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if user.id == 5571722913 or user.id == 5571722913 or user.id == 5571722913:
...
@zedub.zed_cmd(pattern="رفع فؤادي(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
async def permalink(mention):
    ...
    await edit_or_reply(
        mention,
        f"**✾╎المستخـدم ** [{tag}](tg://user?id={user.id}) \n\n**✾╎تـم رفعـه بــ قلبـك .. نبـضك وحبيبك😉 🖤**",
    )


########################  SOURCE ZED ~ BY: ZelZal (@zzzzl1l)  ########################
 
@zedub.zed_cmd(pattern="رفع فأر(?: |$)(.*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝙔𝘼𝙈 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/YamenThon) 𓆪\n\n"
