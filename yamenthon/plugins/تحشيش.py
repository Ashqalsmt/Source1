...
@zedub.on(admin_cmd(pattern="رفع زوجتي(?:\s|$)([\s\S]*)"))
...
@zedub.on(admin_cmd(pattern="رفع كلب(?:\s|$)([\s\S]*)"))
async def permalink(mention):
...
@zedub.on(admin_cmd(pattern="رفع تاجي(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.on(admin_cmd(pattern="رفع مركبه(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.on(admin_cmd(pattern="رفع فرخه(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.on(admin_cmd(pattern="رفع حلوه(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.on(admin_cmd(pattern="رفع حلو(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.on(admin_cmd(pattern="رفع كوده(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.on(admin_cmd(pattern="رفع منشى(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
