...
@zedub.zed_cmd(pattern="رفع زوجتي(?:\s|$)([\s\S]*)")
...
@zedub.zed_cmd(pattern="رفع كلب(?:\s|$)([\s\S]*)")
async def permalink(mention):
    ...
    if user.id == 5571722913:
        return await edit_or_reply(mention, f"**- ⌔╎احترم نفسك هاذا المطور يا تعبان 🛡 يا تعبان 😂**")
...
@zedub.zed_cmd(pattern="رفع تاجي(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع قرد(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع بكلبي(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع مركبه(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    if user.id == 5571722913:
        return await edit_or_reply(mention, f"**- ⌔╎احترم نفسك هاذا المطور يا تعبان 🛡**")
    jepthon = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
...
@zedub.zed_cmd(pattern="رفع زوجي(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع زاحف(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع كحبة(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع فرخه(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع حلوه(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ المستخدم** [{jepthon}](tg://user?id={user.id}) \n**᯽︙  تـم رفعـها حلوه الكروب 🤤😻 بواسطة :** {my_mention} \n**᯽︙  تعاي يعافيتي اريد حضن دافي 😽**")

@zedub.zed_cmd(pattern="رفع هايشة(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع حلو(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع زبال(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع كوده(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع ديوث(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع مميز(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع ادمن(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع منشى(?:\s|$)([\s\S]*)")
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@zedub.zed_cmd(pattern="رفع مجنب(?:\s|$)([\s\S]*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@zedub.zed_cmd(pattern="رفع وصخ(?:\s|$)([\s\S]*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@zedub.zed_cmd(pattern="زواج(?:\s|$)([\s\S]*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
...
@zedub.zed_cmd(pattern="طلاك(?:\s|$)([\s\S]*)")
async def permalink(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
