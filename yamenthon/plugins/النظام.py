...
async def psu(event):
    ...
    softw = "** 𓆩 𝑺𝑶𝑼𝑹𝑪𝑬 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 𝑺𝒀𝑺𝑻𝑬𝑴 𝑰𝑵𝑭𝑶 𓆪 **\n"
    softw += f"**    ⃟⁞⃟⟢ ╎النظام : ** `{uname.system}`\n"
    softw += f"**    ⃟⁞⃟⟢ ╎المرجع  : ** `{uname.release}`\n"
    softw += f"**    ⃟⁞⃟⟢ ╎الاصدار  : ** `{uname.version}`\n"
    softw += f"**    ⃟⁞⃟⟢ ╎النـوع  : ** `{uname.machine}`\n"
    # Boot Time
    boot_time_timestamp = psutil.boot_time()
    bt = datetime.fromtimestamp(boot_time_timestamp)
    softw += f"**    ⃟⁞⃟⟢ ╎تاريـخ التنصيب : **\n**- التاريـخ 📋:**\t`{bt.day}/{bt.month}/{bt.year}`\n**- الـوقت ⏰:**\t`{bt.hour}:{bt.minute}`\n"
    # CPU Cores
    cpuu = "**- معلومات المعالـج :**\n"
    cpuu += "**    ⃟⁞⃟⟢ ╎الماديـه   :** `" + str(psutil.cpu_count(logical=False)) + "`\n"
    cpuu += "**    ⃟⁞⃟⟢ ╎الكليـه      :** `" + str(psutil.cpu_count(logical=True)) + "`\n"
    # CPU frequencies
    cpufreq = psutil.cpu_freq()
    cpuu += f"**    ⃟⁞⃟⟢ ╎اعلـى تـردد    : ** `{cpufreq.max:.2f}Mhz`\n"
    cpuu += f"**    ⃟⁞⃟⟢ ╎اقـل تـردد    : ** `{cpufreq.min:.2f}Mhz`\n"
    cpuu += f"**    ⃟⁞⃟⟢ ╎التـردد الإفتـراضـي : ** `{cpufreq.current:.2f}Mhz`\n\n"
    # CPU usage
    cpuu += "**- استخدامات المعالج لكل وحده :**\n"
    for i, percentage in enumerate(psutil.cpu_percent(percpu=True)):
        cpuu += f"**    ⃟⁞⃟⟢ ╎كـور {i}  : ** `{percentage}%`\n"
    cpuu += "**- استخدامات المعالج الكليـه :**\n"
    cpuu += f"**    ⃟⁞⃟⟢ ╎الكـليه : ** `{psutil.cpu_percent()}%`\n"
    # RAM Usage
    svmem = psutil.virtual_memory()
    memm = "**- استخدامـات الذاكـره :**\n"
    memm += f"**    ⃟⁞⃟⟢ ╎الكـليه     : ** `{get_size(svmem.total)}`\n"
    memm += f"**    ⃟⁞⃟⟢ ╎الفعليـه : ** `{get_size(svmem.available)}`\n"
    memm += f"**    ⃟⁞⃟⟢ ╎المستخدمـه      : ** `{get_size(svmem.used)}`\n"
    memm += f"**    ⃟⁞⃟⟢ ╎المتاحـه: ** `{svmem.percent}%`\n"
    # Bandwidth Usage
    bw = "**- استخدامات الرفـع والتحميـل :**\n"
    bw += f"**    ⃟⁞⃟⟢ ╎الرفـع  : ** `{get_size(psutil.net_io_counters().bytes_sent)}`\n"
    bw += f"**    ⃟⁞⃟⟢ ╎التحميـل : ** `{get_size(psutil.net_io_counters().bytes_recv)}`\n"
    help_string = f"{str(softw)}\n"
    help_string += f"{str(cpuu)}\n"
    help_string += f"{str(memm)}\n"
    help_string += f"{str(bw)}\n"
    help_string += "**- إصـدار بايثــون & تيليثــون :**\n"
    help_string += f"**    ⃟⁞⃟⟢ ╎بايثـون : ** `{sys.version}`\n"
    help_string += f"**    ⃟⁞⃟⟢ ╎تيليثـون : ** `{__version__}`"
